...
namespace Camera
{
	...
	void Camera::GetRectInNDCSpace(glm::vec3& outMin, glm::vec3& outMax, const glm::mat4& inViewPort) const
	{
		...
		{
			// Width/Height가 '월드 단위 오쏘 크기'라고 가정(픽셀 수가 아님)
...
namespace CameraUtil
{
	...
	// rightHanded = true  → OpenGL/일반 RH lookAt와 동일
	// rightHanded = false → D3D-style LH lookAt
	glm::mat4 CreateViewMatrix(const glm::vec3& pos, const glm::vec3& target, glm::vec3& up, bool rightHanded = false)
	{
