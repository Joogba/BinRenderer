...
namespace BinRenderer
{
	...
	void D3D11RendererAPI::Submit(const DrawCommand& inCmd) {
		// DrawCommands 처리 예정
		DrawCommand cmd = inCmd;

		// ─── Uniform 업데이트 ─── 임시코드
			// 최초 Submit 시점에 modelMatrix, viewProj 업데이트
		auto* material = m_materialRegistry->Get(cmd.materialHandle);
		if (material)
		{
			material->GetUniformSet().Set(
				"modelMatrix",
				&cmd.transform,
	...
	// 1) 뷰ID(상위 8비트)
		...
		// 4) 거리 계산 (올바른 예)
		{
			...
			// near=0.1, far=100 매핑 (프로젝션 파라미터와 일치)
