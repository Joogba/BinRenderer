...
void Ex10_Example::handleKeyInput(int key, int scancode, int action, int mods)
{
    if (action == GLFW_PRESS) {
        ...
        case GLFW_KEY_ESCAPE:
            ...
            break;
        case GLFW_KEY_F2:
            if (camera_.type == BinRenderer::Vulkan::Camera::CameraType::lookat) {
                camera_.type = BinRenderer::Vulkan::Camera::CameraType::firstperson;
            }
            else {
                camera_.type = BinRenderer::Vulkan::Camera::CameraType::lookat;
            }
            break;
        }

        // First person camera controls
        if (camera_.type == BinRenderer::Vulkan::Camera::firstperson) {
            switch (key) {
            case GLFW_KEY_W:
